...
*/
	...
	add_filter( 'sp_default_content_filters', 'sp_preprocess_shortcode', 7 );
	//Shortcodes in text widget
	add_filter( 'widget_text', 'sp_preprocess_shortcode', 7 );
	add_filter( 'widget_text', 'shortcode_unautop');
	add_filter( 'widget_text', 'do_shortcode' );
	//Fixes HTML issues created by wpautop
	add_filter( 'the_content', 'sp_shortcode_paragraph_insertion_fix' );
	add_filter( 'widget_text', 'sp_widget_text_shortcode' );// Allow Shortcodes in Sidebar Widgets
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_paragraph_insertion_fix' ) ) {
		function sp_shortcode_paragraph_insertion_fix( $content ) {
			$fix = array(
				'<p>['            => '[',
				']</p>'           => ']',
				']<br />'         => ']',
	...
	} // /sp_shortcode_paragraph_insertion_fix
	...
	*/
	if ( ! function_exists( 'sp_preprocess_shortcode' ) ) {
		function sp_preprocess_shortcode( $content ) {
			...
			add_shortcode( 'raw', 'sp_shortcode_raw' );
			add_shortcode( 'pre', 'sp_shortcode_raw' );
			...
			*/
			add_shortcode( 'button', 'sp_shortcode_button' );
			add_shortcode( 'dropcap', 'sp_shortcode_dropcap' );
			add_shortcode( 'marker', 'sp_shortcode_marker' );
			add_shortcode( 'icon', 'sp_shortcode_icon' );
			add_shortcode( 'last_update', 'sp_shortcode_posts_update' );
			add_shortcode( 'social', 'sp_shortcode_social' );
			//text shortcodes:
				add_shortcode( 'big_text', 'sp_shortcode_big_text' );
				add_shortcode( 'huge_text', 'sp_shortcode_huge_text' );
				add_shortcode( 'small_text', 'sp_shortcode_small_text' );
				add_shortcode( 'uppercase', 'sp_shortcode_uppercase' );
	...
	} // /sp_preprocess_shortcode


	// Enable shortcodes in text widgets
	function sp_widget_text_shortcode( $content ) {
		$content = do_shortcode( $content );
		$new_content = '';
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_administrator' ) ) {
		function sp_shortcode_administrator( $atts, $content = null ) {
			if ( current_user_can( 'edit_dashboard' ) )
				return do_shortcode( $content );
		}
	} // /sp_shortcode_administrator
	add_shortcode( 'administrator', 'sp_shortcode_administrator' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_author' ) ) {
		function sp_shortcode_author( $atts, $content = null ) {
			if ( current_user_can( 'edit_published_posts' ) && ! current_user_can( 'read_private_pages' ) )
				return do_shortcode( $content );
		}
	} // /sp_shortcode_author
	add_shortcode( 'author', 'sp_shortcode_author' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_contributor' ) ) {
		function sp_shortcode_contributor( $atts, $content = null ) {
			if ( current_user_can( 'edit_posts' ) && ! current_user_can( 'delete_published_posts' ) )
				return do_shortcode( $content );
		}
	} // /sp_shortcode_contributor
	add_shortcode( 'contributor', 'sp_shortcode_contributor' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_editor' ) ) {
		function sp_shortcode_editor( $atts, $content = null ) {
			if ( current_user_can( 'moderate_comments' ) && ! current_user_can( 'edit_dashboard' ) )
				return do_shortcode( $content );
		}
	} // /sp_shortcode_editor
	add_shortcode( 'editor', 'sp_shortcode_editor' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_subscriber' ) ) {
		function sp_shortcode_subscriber( $atts, $content = null ) {
			if ( current_user_can('moderate_comments') && ! current_user_can( 'delete_posts' ) )
				return do_shortcode( $content );
		}
	} // /sp_shortcode_subscriber
	add_shortcode( 'subscriber', 'sp_shortcode_subscriber' );
	...
	* Mobile portrait = 479-
	*/
	if ( ! function_exists( 'sp_shortcode_screen' ) ) {
		function sp_shortcode_screen( $atts, $content = null ) {
	...
	add_shortcode( 'screen', 'sp_shortcode_screen' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_accordion' ) ) {
		function sp_shortcode_accordion( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'auto' => ''
				), $atts)
				);
			...
			//output
			$out = '<div class="accordion-wrapper apply-top-margin' . $auto . '"><ul>' . do_shortcode( $content ) . '</ul></div>';
			return $out;
		}
	} // /sp_shortcode_accordion
	add_shortcode( 'accordion', 'sp_shortcode_accordion' );
	...
	* @param title [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_accordion_item' ) ) {
		function sp_shortcode_accordion_item( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'title' => ''
				), $atts)
				);
	...
	} // /sp_shortcode_accordion_item
	add_shortcode( 'accordion_item', 'sp_shortcode_accordion_item' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_box' ) ) {
		function sp_shortcode_box( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'color'       => 'gray',
				'icon'        => '',
				'hero'        => '',
	...
	add_shortcode( 'box', 'sp_shortcode_box' );
...
*/
	...
	* @param url        [URL/NONE]
	*/
	if ( ! function_exists( 'sp_shortcode_button' ) ) {
		function sp_shortcode_button( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'align'      => '',
				'color'      => '',
				'icon'       => '',
...
*/
	...
	* @param title        [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_call_to_action' ) ) {
		function sp_shortcode_call_to_action( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'button_color' => 'green',
				'button_text'  => '',
				'button_url'   => '',
	...
	} // /sp_shortcode_call_to_action
	add_shortcode( 'call_to_action', 'sp_shortcode_call_to_action' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_column' ) ) {
		function sp_shortcode_column( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'last' => false,
				'size' => 'one_half',
				), $atts )
			...
			//validation
			$size = ( in_array( trim( $size ), $columnSizes ) ) ? trim( $size ) : ( 'one_half' );
			$last = ( $last ) ? ( ' last' ) : ( '' );

			//output
			$out = '<div class="' . $size . $last . '">' . wpautop( do_shortcode( $content ) ) . '</div>';
			return $out;
		}
	} // /sp_shortcode_column
	add_shortcode( 'column', 'sp_shortcode_column' );

	/* Support for stupidity... */
		//halfs
			if ( ! function_exists( 'sp_shortcode_one_half' ) ) {
				function sp_shortcode_one_half( $atts, $content = null ) {
					extract( shortcode_atts( array(
								'last' => '',
							), $atts )
						);
					$last = ( trim( $last ) ) ? ( ' last' ) : ( '' );
					return do_shortcode( '[column size="one_half' . $last . '"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_half
			add_shortcode( 'one_half', 'sp_shortcode_one_half' );

			if ( ! function_exists( 'sp_shortcode_one_half_last' ) ) {
				function sp_shortcode_one_half_last( $atts, $content = null ) {
					return do_shortcode( '[column size="one_half last"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_half_last
			add_shortcode( 'one_half_last', 'sp_shortcode_one_half_last' );

		//thirds
			if ( ! function_exists( 'sp_shortcode_one_third' ) ) {
				function sp_shortcode_one_third( $atts, $content = null ) {
					extract( shortcode_atts( array(
								'last' => '',
							), $atts )
						);
					$last = ( trim( $last ) ) ? ( ' last' ) : ( '' );
					return do_shortcode( '[column size="one_third' . $last . '"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_third
			add_shortcode( 'one_third', 'sp_shortcode_one_third' );

			if ( ! function_exists( 'sp_shortcode_one_third_last' ) ) {
				function sp_shortcode_one_third_last( $atts, $content = null ) {
					return do_shortcode( '[column size="one_third last"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_third_last
			add_shortcode( 'one_third_last', 'sp_shortcode_one_third_last' );

			if ( ! function_exists( 'sp_shortcode_two_third' ) ) {
				function sp_shortcode_two_third( $atts, $content = null ) {
					extract( shortcode_atts( array(
								'last' => '',
							), $atts )
						);
					$last = ( trim( $last ) ) ? ( ' last' ) : ( '' );
					return do_shortcode( '[column size="two_third' . $last . '"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_two_third
			add_shortcode( 'two_third', 'sp_shortcode_two_third' );

			if ( ! function_exists( 'sp_shortcode_two_third_last' ) ) {
				function sp_shortcode_two_third_last( $atts, $content = null ) {
					return do_shortcode( '[column size="two_third last"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_two_third_last
			add_shortcode( 'two_third_last', 'sp_shortcode_two_third_last' );

		//fourths
			if ( ! function_exists( 'sp_shortcode_one_fourth' ) ) {
				function sp_shortcode_one_fourth( $atts, $content = null ) {
					extract( shortcode_atts( array(
								'last' => '',
							), $atts )
						);
					$last = ( trim( $last ) ) ? ( ' last' ) : ( '' );
					return do_shortcode( '[column size="one_fourth' . $last . '"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_fourth
			add_shortcode( 'one_fourth', 'sp_shortcode_one_fourth' );

			if ( ! function_exists( 'sp_shortcode_one_fourth_last' ) ) {
				function sp_shortcode_one_fourth_last( $atts, $content = null ) {
					return do_shortcode( '[column size="one_fourth last"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_fourth_last
			add_shortcode( 'one_fourth_last', 'sp_shortcode_one_fourth_last' );
			
			
			
			if ( ! function_exists( 'sp_shortcode_two_fourth' ) ) {
				function sp_shortcode_two_fourth( $atts, $content = null ) {
					extract( shortcode_atts( array(
								'last' => '',
							), $atts )
						);
					$last = ( trim( $last ) ) ? ( ' last' ) : ( '' );
					return do_shortcode( '[column size="two_fourth' . $last . '"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_two_fourth
			add_shortcode( 'two_fourth', 'sp_shortcode_two_fourth' );

			if ( ! function_exists( 'sp_shortcode_two_fourth_last' ) ) {
				function sp_shortcode_two_fourth_last( $atts, $content = null ) {
					return do_shortcode( '[column size="two_fourth last"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_two_fourth_last
			add_shortcode( 'two_fourth_last', 'sp_shortcode_two_fourth_last' );
			
			
			

			if ( ! function_exists( 'sp_shortcode_three_fourth' ) ) {
				function sp_shortcode_three_fourth( $atts, $content = null ) {
					extract( shortcode_atts( array(
								'last' => '',
							), $atts )
						);
					$last = ( trim( $last ) ) ? ( ' last' ) : ( '' );
					return do_shortcode( '[column size="three_fourth' . $last . '"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_three_fourth
			add_shortcode( 'three_fourth', 'sp_shortcode_three_fourth' );

			if ( ! function_exists( 'sp_shortcode_three_fourth_last' ) ) {
				function sp_shortcode_three_fourth_last( $atts, $content = null ) {
					return do_shortcode( '[column size="three_fourth last"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_three_fourth_last
			add_shortcode( 'three_fourth_last', 'sp_shortcode_three_fourth_last' );

		//fifths
			if ( ! function_exists( 'sp_shortcode_one_fifth' ) ) {
				function sp_shortcode_one_fifth( $atts, $content = null ) {
					extract( shortcode_atts( array(
								'last' => '',
							), $atts )
						);
					$last = ( trim( $last ) ) ? ( ' last' ) : ( '' );
					return do_shortcode( '[column size="one_fifth' . $last . '"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_fifth
			add_shortcode( 'one_fifth', 'sp_shortcode_one_fifth' );

			if ( ! function_exists( 'sp_shortcode_one_fifth_last' ) ) {
				function sp_shortcode_one_fifth_last( $atts, $content = null ) {
					return do_shortcode( '[column size="one_fifth last"]' . $content . '[/column]' );
				}
			} // /sp_shortcode_one_fifth_last
			add_shortcode( 'one_fifth_last', 'sp_shortcode_one_fifth_last' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_countdown' ) ) {
		function sp_shortcode_countdown( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'size' => 'xl',
				'time' => '',
				), $atts)
				...
				<div class="countdown-timer' . $size . '">
					<div id="countdown-timer-' . absint( ++$countdownIDs ) . '">
						<div class="dash weeks_dash">
							<span class="dash_title">' . __( 'Weeks', 'sptheme' ) . '</span>
							<div class="digit first">0</div>
							<div class="digit">0</div>
						</div>

						<div class="dash days_dash">
							<span class="dash_title">' . __( 'Days', 'sptheme' ) . '</span>
							<div class="digit first">0</div>
							<div class="digit">0</div>
						</div>

						<div class="dash hours_dash">
							<span class="dash_title">' . __( 'Hours', 'sptheme' ) . '</span>
							<div class="digit first">0</div>
							<div class="digit">0</div>
						</div>

						<div class="dash minutes_dash">
							<span class="dash_title">' . __( 'Minutes', 'sptheme' ) . '</span>
							<div class="digit first">0</div>
							<div class="digit">0</div>
						</div>

						<div class="dash seconds_dash">
							<span class="dash_title">' . __( 'Seconds', 'sptheme' ) . '</span>
							<div class="digit first">0</div>
							<div class="digit">0</div>
						</div>
					</div>
	...
	} // /sp_shortcode_countdown
	add_shortcode( 'countdown', 'sp_shortcode_countdown' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_divider' ) ) {
		function sp_shortcode_divider( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'opacity'      => 0,
				'space_after'  => 0,
				'space_before' => 0,
	...
	} // /sp_shortcode_divider
	add_shortcode( 'devider', 'sp_shortcode_divider' ); //legacy...
	add_shortcode( 'divider', 'sp_shortcode_divider' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_dropcap' ) ) {
		function sp_shortcode_dropcap( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'style' => '',
				'type'  => ''
				), $atts )
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_icon' ) ) {
		function sp_shortcode_icon( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'size'  => '',
				'style' => '',
				'type'  => '',
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_list' ) ) {
		function sp_shortcode_list( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'bullet' => '',
				'icon'   => '',
				), $atts )
	...
	add_shortcode( 'list', 'sp_shortcode_list' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_login' ) ) {
		function sp_shortcode_login( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'stay' => false,
				), $atts)
				);
			...
			if ( ! is_user_logged_in() ) {
				$out .= '<h3>' . __( 'Log in', 'sptheme' ) . '</h3>';

				$out .= wp_login_form( array(
					'echo'           => false,
					'redirect'       => $redirect,
					'form_id'        => 'form-login',
					'label_username' => __( 'Username', 'sptheme' ),
					'label_password' => __( 'Password', 'sptheme' ),
					'label_remember' => __( 'Remember Me', 'sptheme' ),
					'label_log_in'   => __( 'Log In', 'sptheme' ),
				...
				$out .= '<p class="note"><small><a href="' . wp_lostpassword_url( get_permalink() ) . '" title="' . __( 'Password will be resent to your e-mail address.', 'sptheme' ) . '">' . __( 'I have lost my password', 'sptheme' ) . '</a></small></p>';
			} else {
				$out .= '[button url="' . wp_logout_url( get_permalink() ) . '" color="red" size="xl"]' . __( 'Log out', 'sptheme' ) . '[/button]';
			}
	...
	add_shortcode( 'login', 'sp_shortcode_login' );
...
*/
	...
	* @param style [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_marker' ) ) {
		function sp_shortcode_marker( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'color' => 'gray',
				'style' => '',
				), $atts )
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_content_module' ) ) {
		function sp_shortcode_content_module( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'id'        => null,
				'layout'    => '',
				'module'    => null,
			...
			if ( $the_module->have_posts() ) {
				...
				$moreURL = esc_url( stripslashes( sp_meta_option( 'module-link' ) ) );
				$iconBg  = ( sp_meta_option( 'module-icon-box-color' ) ) ? ( ' style="background-color: ' . sp_meta_option( 'module-icon-box-color', get_the_ID(), 'color' ) . '; color: ' . sp_modify_color( sp_meta_option( 'module-icon-box-color', get_the_ID(), 'color' ), 200, -200 ) . '"' ) : ( '' );
				$layout  = ( $layout ) ? ( ' layout-' . $layout ) : ( null );

				//HTML to display output
				$classWrapper  = ( 'icon' === sp_meta_option( 'module-type' ) ) ? ( ' icon-module' ) : ( '' );
				...
				$out .= '<div class="content-module-' . get_the_ID() . $classWrapper . $layout . '">';
					...
					if ( 'icon' === sp_meta_option( 'module-type' ) && sp_meta_option( 'module-font-icon' ) ) {
						...
						$out .= '<div class="' . $imageContainerClass . '"' . $iconBg . '>';
							if ( $moreURL )
								$out .= '<a href="' . $moreURL . '">';
							$out .= '<i class="' . sp_meta_option( 'module-font-icon' ) . '"></i>';
							if ( $moreURL )
								$out .= '</a>';
						$out .= '</div>';
						...
						$imageContainerClass  = ( 'icon' === sp_meta_option( 'module-type' ) ) ? ( 'icon-container' ) : ( 'image-container' );
						$imageContainerClass .= ( $iconBg ) ? ( ' colored-background' ) : ( '' );
						$out .= '<div class="' . $imageContainerClass . '"' . $iconBg . '>';
							if ( $moreURL )
								$out .= '<a href="' . $moreURL . '">';
					...
					$out .= '<div class="module-content clearfix">' . apply_filters( 'sp_default_content_filters', get_the_content() ) . '</div>';
	...
	} // /sp_shortcode_content_module
	add_shortcode( 'content_module', 'sp_shortcode_content_module' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_faq' ) ) {
		function sp_shortcode_faq( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'align'        => 'left',
				'category'     => null,
				'desc_size'    => 4,
			...
			/*if ( 'disable' === sp_option( 'cp-role-faq' ) )
				return;*/
			...
			if ( $faq->have_posts() ) {
				...
				while ( $faq->have_posts() ) : //output post content
					...
					$out   .= '<article class="item item-' . get_the_ID() . $termsClass . '"><div class="toggle-wrapper">';
						...
						$out .= '<div class="answer"><h2 class="faq-question">' . get_the_title() . '</h2>' . apply_filters( 'sp_default_content_filters', get_the_content() ) . '</div>';
					$out   .= '</div></article>';
				endwhile;
	...
	add_shortcode( 'faq', 'sp_shortcode_faq' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_posts_update' ) ) {
		function sp_shortcode_posts_update( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'item'   => 'posts',
			...
			if ( 'disable' === sp_option( 'cp-role-projects' ) )
				$itemArray = array(
					'posts' => 'post'
					);
	...
	*/
	if ( ! function_exists( 'sp_shortcode_logos' ) ) {
		function sp_shortcode_logos( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'align'        => 'left',
				'category'     => null,
				'columns'      => 4,
			...
			/*if ( 'disable' === sp_option( 'cp-role-logos' ) )
				return;*/
			global $post;
			$out = '';
			...
			if ( $logos->have_posts() ) {
				...
				//if no description (no shortcode content)
					...
					while ( $logos->have_posts() ) : //output post content
						...
						$link   = ( sp_get_custom_field( 'sp_logos_link', $post->ID ) ) ? ( esc_url( sp_get_custom_field( 'sp_logos_link', $post->ID ) ) ) : ( null );
						...
						$out   .= '<div class="item column col-1' . $cols . ' no-margin">';
							...
							$out .= '<img src="' . esc_url( sp_get_custom_field( 'sp_logos_img', $post->ID ) ) . '" alt="' . get_the_title() . '" title="' . get_the_title() . '"' . $grayscale .' />';
							$out .= ( $link ) ? ( '</a>' ) : ( '' );
						$out   .= '</div>';
					endwhile;
	...
	add_shortcode( 'logos', 'sp_shortcode_logos' );
	...
	* @param thumb          [BOOLEAN]
	*/
	if ( ! function_exists( 'sp_shortcode_posts' ) ) {
		function sp_shortcode_posts( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'align'          => 'left',
				'category'       => null,
				'columns'        => 4,
			...
			$imgSize = ( sp_option( 'general-projects-image-ratio' ) ) ? ( 'mobile-' . sp_option( 'general-projects-image-ratio' ) ) : ( 'mobile-ratio-169' );
				...
				//if no description (no shortcode content)
					...
					while ( $posts->have_posts() ) : //output post content
						...
						if ( 0 < $excerptLength ) {
							...
							$excerptText = sp_string_length( strip_tags( $excerptText ), $excerptLength, '&hellip;' );
						} else {
							$excerptText = '';
						}
						...
						$postOutput  = array(
								...
								'thumb'   => sp_thumb( array(
										'class'       => 'post-thumb',
										'size'        => $imgSize,
										'list'        => true,
										'placeholder' => true,
										'overlay'     => __( 'Read more', 'sptheme' ),
									) ),
						...
						$out   .= '<article class="column col-1' . $cols . ' no-margin">';
							...
							$out .= '<div class="text">';
								...
								$out .= '<div class="text-element post-date">' . $postOutput['date'] . '</div>';
	...
	add_shortcode( 'posts', 'sp_shortcode_posts' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_prices' ) ) {
		function sp_shortcode_prices( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'table' => null
				), $atts )
				);

			if ( 'disable' === sp_option( 'cp-role-prices' ) )
				return;
			...
			if ( $price_table->have_posts() && $table ) {
				...
				while ( $price_table->have_posts() ) :
					...
					$colorBg = ( sp_meta_option( 'price-color' ) ) ? ( sp_meta_option( 'price-color', null, 'color' ) ) : ( null );
					$styles  = ( $colorBg ) ? ( array( ' style="background-color: ' . $colorBg . '; color: ' . sp_modify_color( $colorBg, 200, -200 ) . ';"', ' style="background-color: ' . $colorBg . '; background-image: ' . preg_replace( '/\s+/', ' ', sp_css3_gradient( sp_modify_color( $colorBg, -8, -8 ), 32 ) ) . '; color: ' . sp_modify_color( $colorBg, 200, -200 ) . ';"' ) ) : ( array( '', '' ) );

					$outColumn = '<div class="price-column column no-margin col-1' . $columnSize . sp_meta_option( 'price-style' ) . '">';

						$outColumn .= '<div class="price-heading"' . $styles[0] . '>';
							$outColumn .= '<h3' . $styles[1] . '>' . get_the_title() . '</h3>';
							$outColumn .= '<p class="cost">' . sp_meta_option( 'price-cost' ) . '</p>';
							$outColumn .= '<p class="note">' . sp_meta_option( 'price-note' ) . '</p>';
							$outColumn .= '<div class="price-button">';
								$outColumn .= ( sp_meta_option( 'price-btn-text' ) ) ? ( '<p class="wrap-button">[button url="' . esc_url( sp_meta_option( 'price-btn-url' ) ) . '" color="' . sp_meta_option( 'price-btn-color' ) . '" size="medium"]' . sp_meta_option( 'price-btn-text' ) . '[/button]</p>' ) : ( '' );
							$outColumn .= '</div>';
						$outColumn .= '</div>';

						$outColumn .= '<div class="price-spec">';
						$outColumn .= apply_filters( 'sp_default_content_filters', get_the_content() );
						$outColumn .= '</div>';
					...
					$colOrder = ( sp_meta_option( 'price-order' ) ) ? ( sp_meta_option( 'price-order' ) ) : ( -7 + $i );
	...
	add_shortcode( 'prices', 'sp_shortcode_prices' );
	...
	* @param thumb          [BOOLEAN]
	*/
	if ( ! function_exists( 'sp_shortcode_projects' ) ) {
		function sp_shortcode_projects( $atts, $content = null ) {
			global $paged;
			...
			if ( empty( $projectIcons ) )
				$projectIcons = array(
						'static-project' => array( 'wmicon-image', __( 'Image', 'sptheme' ) ),
						'slider-project' => array( 'wmicon-gallery', __( 'Image slider', 'sptheme' ) ),
						'video-project'  => array( 'wmicon-video', __( 'Video', 'sptheme' ) ),
						'audio-project'  => array( 'wmicon-audio', __( 'Audio', 'sptheme' ) )
					);
			...
			$pagination      = ( $pagination ) ? ( sp_pagination( $projects, array( 'print' => false ) ) ) : ( '' );
			...
			if ( $projects->have_posts() ) {
				...
				$out .= ( $related ) ? ( '<h3 class="separator-heading"><span class="text-holder">' . strip_tags( trim( $related ), '<strong><span><small><em><b><i>' ) . '</span></h3>' ) : ( '' );
				...
				if ( $filter ) {
					...
					//if parent category set - filter from child categories
						...
						$filterContent .= '<li><a href="#" data-filter="*" class="active">' . sprintf( __( 'All <span>%s</span>', 'sptheme' ), $parentCategory->name ) . '</a></li>';
					...
					//no parent category - filter from all categories
						...
						$filterContent .= '<li><a href="#" data-filter="*" class="active">' . __( 'All', 'sptheme' ) . '</a></li>';
				...
				//if no description (no shortcode content)
					...
					while ( $projects->have_posts() ) : //output post content
						...
						$link        = ( sp_meta_option( 'project-link-list' ) ) ? ( esc_url( sp_meta_option( 'project-link' ) ) ) : ( get_permalink() );
						$target      = ( 'target-blank' == sp_meta_option( 'project-link-list' ) ) ? ( ' target="_blank"' ) : ( '' );
						$overlayText = ( 'modal' == sp_meta_option( 'project-link-list' ) ) ? ( __( 'Zoom', 'sptheme' ) ) : ( __( 'View', 'sptheme' ) );

						if ( 0 < $excerptLength && has_excerpt() )
							$excerptText = sp_string_length( get_the_excerpt(), $excerptLength, '&hellip;' );
						else
							$excerptText = '';

						$icon      = 'wmicon-image';
						$iconTitle = __( 'Image', 'sptheme' );
						if ( sp_meta_option( 'project-type' ) ) {
							if ( isset( $projectIcons[sp_meta_option( 'project-type' )][0] ) )
								$icon = $projectIcons[sp_meta_option( 'project-type' )][0];
							if ( isset( $projectIcons[sp_meta_option( 'project-type' )][1] ) )
								$iconTitle = $projectIcons[sp_meta_option( 'project-type' )][1];
						}
						...
						$projectOutput  = array(
								...
								'thumb'    => sp_thumb( array(
										...
										'size'         => $imgSize,
										'list'         => true,
										'link'         => array( $link, sp_meta_option( 'project-link-list' ) ),
										'placeholder'  => true,
										'a-attributes' => $target,
										'overlay'      => $overlayText,
									) ),
								'title'    => '<h3 class="project-title text-element"><a href="' . $link . '"' . $target . ' class="' . sp_meta_option( 'project-link-list' ) . '">' . get_the_title() . '</a></h3>',
								'type'     => '<a class="project-icon ' . sp_meta_option( 'project-link-list' ) . '"' . $target . ' href="' . $link . '"><i class="' . $icon . '" title="' . $iconTitle . '"></i></a>',
	...
	} // /sp_shortcode_projects
	add_shortcode( 'projects', 'sp_shortcode_projects' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_project_attributes' ) ) {
		function sp_shortcode_project_attributes( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'title' => ''
				), $atts )
				);
			...
			if ( sp_meta_option( 'project-link' ) ) {
				$link = sp_meta_option( 'project-link' );
				$out .= '<li class="attribute-link"><strong class="attribute-heading">' . __( 'Project URL', 'sptheme' ) . ':</strong> ';
				$out .= '<a href="' . esc_url( $link ) . '">' . $link . '</a></li>';
			}

			if ( sp_meta_option( 'project-attributes' ) ) {
				foreach ( sp_meta_option( 'project-attributes' ) as $item ) {
	...
	} // /sp_shortcode_project_attributes
	add_shortcode( 'project_attributes', 'sp_shortcode_project_attributes' );
	...
	* @param thumb      [BOOLEAN]
	*/
	if ( ! function_exists( 'sp_shortcode_staff' ) ) {
		function sp_shortcode_staff( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'align'      => 'left',
				'columns'    => 4,
				'count'      => 4,
			...
			if ( 'disable' === sp_option( 'cp-role-staff' ) )
				return;

			$out = '';

			$imgSize = ( sp_option( 'general-staff-image-ratio' ) ) ? ( 'mobile-' . sp_option( 'general-staff-image-ratio' ) ) : ( 'mobile-ratio-169' );
			...
			if ( $staff->have_posts() ) {
				...
				//if no description (no shortcode content)
					...
					while ( $staff->have_posts() ) : //output post content
						...
						if ( sp_option( 'cp-staff-rich' ) )
							$excerptText  = ( has_excerpt() ) ? ( apply_filters( 'sp_default_content_filters', get_the_excerpt() ) ) : ( '' );
 						else
							$excerptText = apply_filters( 'sp_default_content_filters', get_the_content() );

						$contacts = '';
						if ( sp_meta_option( 'staff-phone' ) )
							$contacts .= '<li class="staff-phone">' . sp_meta_option( 'staff-phone' ) . '</li>';
						if ( sp_meta_option( 'staff-email' ) )
							$contacts .= '<li class="staff-email"><a href="#" data-address="' . sp_nospam( sp_meta_option( 'staff-email' ) ) . '" class="email-nospam">' . sp_nospam( sp_meta_option( 'staff-email' ) ) . '</a></li>';
						if ( sp_meta_option( 'staff-linkedin' ) )
							$contacts .= '<li class="staff-linkedin"><a href="' . esc_url( sp_meta_option( 'staff-linkedin' ) ) . '" target="_blank">' . get_the_title() . '</a></li>';
						if ( sp_meta_option( 'staff-skype' ) )
							$contacts .= '<li class="staff-skype"><a href="skype:' . sanitize_title( sp_meta_option( 'staff-skype' ) ) . '?call">' . sp_meta_option( 'staff-skype' ) . '</a></li>';
						...
						$staffOutput  = array(
								'thumb' => sp_thumb( array(
										...
										'link'        => 'modal',
										'placeholder' => true,
										'overlay'     => __( 'Zoom', 'sptheme' ),
									) ),
								'thumb-permalink' => sp_thumb( array(
										'class'       => 'staff-thumb',
										'size'        => $imgSize,
										'list'        => true,
										'placeholder' => true,
								...
								'position'   => '<p class="staff-position text-element">' . sp_meta_option( 'staff-position' ) . '</p>',
								'excerpt'    => $excerptText
							);
						...
						$out   .= '<article class="column col-1' . $cols . ' no-margin">';
							if ( $thumb )
								$out .= ( sp_option( 'cp-staff-rich' ) ) ? ( $staffOutput['thumb-permalink'] ) : ( $staffOutput['thumb'] );
							$out .= '<div class="text">';
								$out .= ( sp_option( 'cp-staff-rich' ) ) ? ( $staffOutput['title-link'] ) : ( $staffOutput['title'] );
	...
	add_shortcode( 'staff', 'sp_shortcode_staff' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_subpages' ) ) {
		function sp_shortcode_subpages( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'depth'   => 0,
				'order'   => 'menu',
				'parents' => false,
	...
	} // /sp_shortcode_subpages
	add_shortcode( 'subpages', 'sp_shortcode_subpages' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_testimonials' ) ) {
		function sp_shortcode_testimonials( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'category' => null,
				'count'    => 5,
				'layout'   => 'normal',
			...
			if ( sp_option( 'blog-no-format-quote' ) || ! $category )
				return;
			...
			if ( $testimonials->have_posts() ) {
				...
				while ( $testimonials->have_posts() ) :
					...
					if (
						...
						$out .= '<blockquote class="simple">' . apply_filters( 'sp_default_content_filters', $quote[0] ) . '</blockquote>';

						$quoteSource = '';
						if ( has_post_thumbnail() ) {
							$imgUrl       = wp_get_attachment_image_src( get_post_thumbnail_id(), 'widget' );
							$escImgAlt    = esc_attr( strip_tags( strip_shortcodes( sp_meta_option( 'quoted-author' ) ) ) );
	...
	} // /sp_shortcode_testimonials
	add_shortcode( 'testimonials', 'sp_shortcode_testimonials' );
...
*/
	...
	* @param style [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_pullquote' ) ) {
		function sp_shortcode_pullquote( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'align' => 'left',
				'style' => '',
				), $atts )
	...
	} // /sp_shortcode_pullquote
	add_shortcode( 'pullquote', 'sp_shortcode_pullquote' );
...
*/
	...
	* [pre]content[/pre]
	*/
	if ( ! function_exists( 'sp_shortcode_raw' ) ) {
		function sp_shortcode_raw( $atts, $content = null ) {
...
*/
	...
	* @param time [#/NONE]
	*/
	if ( ! function_exists( 'sp_shortcode_simple_slideshow' ) ) {
		function sp_shortcode_simple_slideshow( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'ids'  => '',
				'link' => '',
				'size' => 'ratio-169',
			...
			foreach ( $ids as $imageId ) {
				$fullImage = wp_get_attachment_image_src( $imageId, sp_option( 'general-lightbox-img' ) );
	...
	} // /sp_shortcode_simple_slideshow
	add_shortcode( 'simple_slideshow', 'sp_shortcode_simple_slideshow' );
...
*/
	...
	* @param url   [URL]
	*/
	if ( ! function_exists( 'sp_shortcode_social' ) ) {
		function sp_shortcode_social( $atts, $content = null ) {
...
*/
	...
	*
	* Home page template split
	*
	* @param class [PREDEFINED TEXT]
	* @param style [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_section' ) ) {
		function sp_shortcode_section( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'class' => '',
				'style' => '',
				), $atts)
	...
	} // /sp_shortcode_section
	add_shortcode( 'section', 'sp_shortcode_section' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_table' ) ) {
		function sp_shortcode_table( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'class'       => '',
				'cols'        => '',
				'data'        => '',
	...
	add_shortcode( 'table', 'sp_shortcode_table' );
	...
	* Table row
	*/
	if ( ! function_exists( 'sp_shortcode_table_row' ) ) {
		function sp_shortcode_table_row( $atts, $content = null ) {
			//output
			return '<tr>' . do_shortcode( $content ) . '</tr>';
		}
	} // /sp_shortcode_table_row
	add_shortcode( 'trow', 'sp_shortcode_table_row' );
	...
	* Table row altered
	*/
	if ( ! function_exists( 'sp_shortcode_table_row_alt' ) ) {
		function sp_shortcode_table_row_alt( $atts, $content = null ) {
			//output
			return '<tr class="alt">' . do_shortcode( $content ) . '</tr>';
		}
	} // /sp_shortcode_table_row_alt
	add_shortcode( 'trow_alt', 'sp_shortcode_table_row_alt' );
	...
	* Table cell
	*/
	if ( ! function_exists( 'sp_shortcode_table_cell' ) ) {
		function sp_shortcode_table_cell( $atts, $content = null ) {
			//output
			return '<td>' . do_shortcode( $content ) . '</td>';
		}
	} // /sp_shortcode_table_cell
	add_shortcode( 'tcell', 'sp_shortcode_table_cell' );
	...
	* Table heading cell
	*/
	if ( ! function_exists( 'sp_shortcode_table_cell_heading' ) ) {
		function sp_shortcode_table_cell_heading( $atts, $content = null ) {
			//output
			return '<th>' . do_shortcode( $content ) . '</th>';
		}
	} // /sp_shortcode_table_cell_heading
	add_shortcode( 'tcell_heading', 'sp_shortcode_table_cell_heading' );
...
*/
	/**
	* [tabs type="normal"]content[/tabs]
	*
	* Tabs wrapper
	*
	* @param type [normal/modern/classic]
	*/
	if ( ! function_exists( 'sp_shortcode_tabs' ) ) {
		function sp_shortcode_tabs( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'type' => ''
				), $atts)
				);
			$types = array( 'normal-tabs', 'modern-tabs', 'classics-tabs' );

			$type = ( in_array( trim( $type ), $types ) ) ? ( ' ' . trim( $type ) ) : ( ' normal-tabs' );

			//output
			$out = '<div class="' . $type . '"><ul class="tabs-nav">' . do_shortcode( $content ) . '</ul></div>';
			return $out;
		}
	} // /sp_shortcode_tabs
	add_shortcode( 'tabs', 'sp_shortcode_tabs' );
	...
	*/
	if ( ! function_exists( 'sp_shortcode_tab' ) ) {
		function sp_shortcode_tab( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'title' => '',
				), $atts)
				);
	...
	add_shortcode( 'tab', 'sp_shortcode_tab' );
...
*/
	...
	* @param style [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_big_text' ) ) {
		function sp_shortcode_big_text( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'style' => '',
				), $atts)
				);
	...
	* @param type [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_separator_heading' ) ) {
		function sp_shortcode_separator_heading( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'align' => '',
				'id'    => '',
				'size'  => 2,
			...
			//validation
			$aligns = array( 'center', 'right' );

			$align = ( in_array( trim( $align ), $aligns ) ) ? ( ' text-' . trim( $align ) ) : ( '' );
			...
			//output
			return '<h' . $size . $id . ' class="separator-heading' . esc_attr( $type ) . esc_attr( $align ) . '"><span class="text-holder">' . do_shortcode( strip_tags( trim( $content ), '<img><strong><span><small><em><b><i>' ) ) . '</span></h' . $size . '>';
		}
	} // /sp_shortcode_separator_heading
	add_shortcode( 'separator_heading', 'sp_shortcode_separator_heading' );
	...
	* @param style [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_huge_text' ) ) {
		function sp_shortcode_huge_text( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'style' => '',
				), $atts)
				);
	...
	* @param style [TEXT]
	*/
	if ( ! function_exists( 'sp_shortcode_small_text' ) ) {
		function sp_shortcode_small_text( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'style' => '',
				), $atts)
				);
	...
	* Uppercase
	*/
	if ( ! function_exists( 'sp_shortcode_uppercase' ) ) {
		function sp_shortcode_uppercase( $atts, $content = null ) {
			return do_shortcode( '<span class="uppercase">' . $content . '</span>' );
		}
	} // /sp_shortcode_uppercase
	add_shortcode( 'uppercase', 'sp_shortcode_uppercase' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_toggle' ) ) {
		function sp_shortcode_toggle( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'open'  => false,
				'title' => '',
				), $atts)
	...
	add_shortcode( 'toggle', 'sp_shortcode_toggle' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_video' ) ) {
		function sp_shortcode_video( $atts, $content = null ) {
			global $is_IE;
			...
			//http://www.screenr.com/ScrID
				...
				$out = '<div class="entry-video"><iframe src="' . esc_url( $url ) . '"' . $iFrameAtts . '></iframe></div>';
			} elseif ( $url ) {
			//all other embeds
				$out = '<div class="entry-video">' . wp_oembed_get( esc_url( $url ) ) . '</div>';
			}

			//output
			if ( ! $out )
				$out = do_shortcode( '[box color="red" icon="warning"]' . __( 'Please check the video URL', 'sptheme' ) . '[/box]' );

			return $out;
		}
	} // /sp_shortcode_video
	add_shortcode( 'video', 'sp_shortcode_video' );
...
*/
	...
	*/
	if ( ! function_exists( 'sp_shortcode_widget_area' ) ) {
		function sp_shortcode_widget_area( $atts, $content = null ) {
			extract( shortcode_atts( array(
				'area'   => '',
				'layout' => 'horizontal',
				), $atts )
			...
			$areas   = array_flip( sp_widget_areas() );
			$layouts = array(
				'horizontal'    => 'columns',
				'vertical'      => 'vertical',
			...
			//sp_sidebar($defaultSidebar, $class, $restrictCount, $print)
			if ( $area )
				return sp_sidebar( $area, $class, $restrictedCount, false );
		}
	} // /sp_shortcode_widget_area
	add_shortcode( 'widgets', 'sp_shortcode_widget_area' );
